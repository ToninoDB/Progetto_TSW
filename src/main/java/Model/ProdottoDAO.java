...
public class ProdottoDAO extends HttpServlet {
    public static ArrayList<Prodotto> doRetriveByCategoria(String categoria) {
        ...
        try (Connection con = ConPool.getConnection()) {
            PreparedStatement ps =
                    con.prepareStatement("SELECT idProdotto, nomeCategoria, nomeProd, descrizione, taglia, prezzo, quantita FROM Prodotto WHERE nomeCategoria LIKE ?");
            ...
            while (rs.next()) {
                ...
                p.setTaglia(rs.getInt(5));
                p.setPrezzo(rs.getDouble(6));
    ...
    public static ArrayList<Prodotto> doRetriveAll() {
        ...
        try (Connection con = ConPool.getConnection()) {
            PreparedStatement ps =
                    con.prepareStatement("SELECT idProdotto, nomeCategoria, nomeProd, descrizione, taglia, prezzo, quantita FROM Prodotto");
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                Prodotto p = new Prodotto();
                p.setIdProdotto(rs.getString(1));
    ...
    public static ArrayList<Prodotto> doRetriveByFilter(String categoria, double max, double min) {
        ...
        try (Connection con = ConPool.getConnection()) {
            PreparedStatement ps =
                    con.prepareStatement("SELECT idProdotto, nomeCategoria, nomeProd, descrizione, taglia, prezzo, quantita FROM Prodotto WHERE nomeCategoria LIKE ? AND prezzo>= ? AND prezzo<= ?");
    ...
    public static Prodotto doRetriveBySearch(String nome) {
        ...
        try (Connection con = ConPool.getConnection()) {
            PreparedStatement ps =
                    con.prepareStatement("SELECT idProdotto, nomeCategoria, nomeProd, descrizione, taglia, prezzo, quantita FROM Prodotto WHERE upper(nomeProd) LIKE ?");
    ...
    public static ArrayList<Prodotto> doRetriveQuantitaEsaurita() {
        ...
        try (Connection con = ConPool.getConnection()) {
            PreparedStatement ps =
                    con.prepareStatement("SELECT idProdotto, nomeCategoria, nomeProd, descrizione, taglia, prezzo, quantita FROM Prodotto WHERE quantita=0");
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                Prodotto p = new Prodotto();
                p.setIdProdotto(rs.getString(1));
    ...
    public static void aggiuntaProdotto(Prodotto p) {
        try (Connection con = ConPool.getConnection()) {
            PreparedStatement ps = con.prepareStatement("INSERT INTO Prodotto (idProdotto, nomeCategoria, nomeProd, descrizione, taglia, prezzo, quantita) VALUES (?,?,?,?,?,?,?)",
                    Statement.RETURN_GENERATED_KEYS);
    ...
    public static ArrayList<Prodotto> getProdottiRandom(int n) throws SQLException {
        ArrayList<Prodotto> randomProdotti = new ArrayList<>();
        String query = "SELECT * FROM Prodotto ORDER BY RAND() LIMIT ?";
